...
ResetAllMenus(void)
{
    ...
    CheckMenuItem(menuMain, IDM_VID_SDL_SR_OPENGL, MF_UNCHECKED);  
...
MainWindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        ...
        case WM_ACTIVATEAPP:
            ...
            if (video_fullscreen & 1 && wParam == FALSE && vid_api < 6)
                PostMessage(hwndMain, WM_LEAVEFULLSCREEN, 0, 0);
            break;
...
ui_init(int nCmdShow)
{
    ...
    if (start_in_fullscreen || vid_api == 4 || vid_api == 5) //psakhis: start on fullscreen for 15khz
