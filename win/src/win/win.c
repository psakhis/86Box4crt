...
#include <86box/win_sdl_sr.h> //psakhis
...
static const struct {
    ...
    { "SDL_SR_OpenGL", 1, (int (*)(void *)) sdl_sr_initho, sdl_sr_close, NULL, sdl_sr_pause, sdl_sr_enable, sdl_sr_set_fs, sdl_sr_reload }    
#ifdef USE_VNC
    ,
...
plat_setfullscreen(int on)
{
    ...
    if (on && video_fullscreen_first) {
        video_fullscreen |= 2;
        if (vid_api != 4 && vid_api != 5) //psakhis (disable fu.. dialog)
